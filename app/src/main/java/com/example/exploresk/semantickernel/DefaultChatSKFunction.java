...
public class DefaultChatSKFunction extends DefaultSemanticSKFunction<CompletionRequestSettings> implements CompletionSKFunction {

    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultChatSKFunction.class);
    ...
    public DefaultChatSKFunction(List<ParameterView> parameters, String skillName, String functionName, String description, CompletionRequestSettings requestSettings, SemanticFunctionConfig functionConfig, @Nullable KernelSkillsSupplier kernelSkillsSupplier) {
    ...
    */
    ...
    private SKContext buildContext() {
        ...
        return SKBuilders.context().setVariables(SKBuilders.variables().build()).setSkills(getSkillsSupplier() == null ? null : getSkillsSupplier().get()).build();
    }
    ...
    public Mono<SKContext> aggregatePartitionedResultsAsync(List<String> partitionedInput, @Nullable SKContext contextIn) {
        ...
        // function
        BiFunction<Flux<SKContext>, String, Flux<SKContext>> executeNextChunk = (contextInput, input) -> contextInput.concatMap(newContext -> {
            SKContext updated = newContext.copy().update(input);
            return invokeAsync(updated, null);
        });

        Flux<SKContext> results = Flux.fromIterable(partitionedInput).scanWith(() -> Flux.just(context), executeNextChunk).skip(1) // Skip the first element, which is the initial context
            .concatMap(it -> it);
    ...
    protected Mono<SKContext> invokeAsyncInternal(SKContext context, @Nullable CompletionRequestSettings settings) {
        ...
        return function.run(this.aiService.get(), finalSettings, context).map(result -> {
    ...
    public void registerOnKernel(Kernel kernel) {
        this.function = (TextCompletion client, CompletionRequestSettings requestSettings, SKContext contextInput) -> {
            ...
            System.out.println("This print function works");

            if (client instanceof ChatCompletion) {
                System.out.println("enter stream flow");
                func.renderAsync(context).flatMapMany(prompt -> {
                        LOGGER.debug("RENDERED PROMPT: \n" + prompt);
                        return ((ChatCompletion<OpenAIChatHistory>) client).generateMessageStream(new OpenAIChatHistory(prompt), new ChatRequestSettings(requestSettings));
                    })
                    .filter(completion -> !completion.isEmpty())
//                    .take(1)
//                    .single()
                    .map(x -> context.update(context.getResult() + x))
                    .blockLast();
...
//                context.update(r);
                ...
                System.out.println("enter non-stream flow");
                return func.renderAsync(context).flatMapMany(prompt -> {
                        LOGGER.debug("RENDERED PROMPT: \n" + prompt);
                        return client.completeAsync(prompt, requestSettings);
                    })
                    .single().map(completion -> {
                        return context.update(completion.get(0));
                    })
                    .doOnError(ex -> {
                        LOGGER.warn("Something went wrong while rendering the semantic" + " function or while executing the text" + " completion. Function: {}.{}. Error: {}", getSkillName(), getName(), ex.getMessage());
    ...
    public static class Builder implements CompletionSKFunction.Builder {
        ...
        public CompletionSKFunction build() {
            if (kernel == null) {
                throw new AIException(AIException.ErrorCodes.InvalidConfiguration, "Called builder to create a function without setting the kernel");
            }
            ...
            if (functionConfig == null) {
                ...
                if (promptTemplate == null) {
                    throw new AIException(AIException.ErrorCodes.InvalidConfiguration, "Must set prompt template before building");
                }

                if (promptTemplateConfig == null) {
                    promptTemplateConfig = new PromptTemplateConfig(description, "completion", completionConfig);
                }

                PromptTemplate template = SKBuilders.promptTemplate().setPromptTemplateConfig(promptTemplateConfig).setPromptTemplate(promptTemplate).setPromptTemplateEngine(kernel.getPromptTemplateEngine()).build();

                // Prepare lambda wrapping AI logic
                functionConfig = new SemanticFunctionConfig(promptTemplateConfig, template);
            }

            CompletionRequestSettings requestSettings = CompletionRequestSettings.fromCompletionConfig(functionConfig.getConfig().getCompletionConfig());

            PromptTemplate promptTemplate = functionConfig.getTemplate();

            DefaultChatSKFunction function = new DefaultChatSKFunction(promptTemplate.getParameters(), skillName, functionName, functionConfig.getConfig().getDescription(), requestSettings, functionConfig, null);
        ...
        @Override
        public CompletionSKFunction.Builder setPromptTemplateConfig(PromptTemplateConfig promptTemplateConfig) {
            this.promptTemplateConfig = promptTemplateConfig;
            return this;
        }

        @Override
        public CompletionSKFunction.Builder setCompletionConfig(PromptTemplateConfig.CompletionConfig completionConfig) {
            this.completionConfig = completionConfig;
            return this;
        }

        @Override
        public CompletionSKFunction.Builder setSemanticFunctionConfig(SemanticFunctionConfig functionConfig) {
            this.functionConfig = functionConfig;
            return this;
        }
